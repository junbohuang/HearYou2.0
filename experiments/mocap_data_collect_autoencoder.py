...
def get_raw_waveform(path_to_wav_sentences, f):
    wav = get_audio(path_to_wav_sentences, f + '.wav')
    return wav[-1]

def get_duration(path_to_wav_sentences, f):
    wav = get_audio(path_to_wav_sentences, f + '.wav')
    duration = wav[0][3]/wav[0][2]
    return duration

def get_spectrogram(path_to_wav_sentences, f):

    sample_rate, samples = wavfile.read(path_to_wav_sentences+ f + '.wav')
...
def read_iemocap_mocap():
    ...
    for session in sessions:
        ...
        path_to_wav_sentences = data_path + session + '/sentences/wav/'
        ...
        for f in files:       
            ...
            for ie, e in enumerate(emotions):
                ...
                e['durations'] = get_duration(path_to_wav_sentences + e['id'][:-5] + '/', e['id'])
                e['raw_waveforms'] = get_raw_waveform(path_to_wav_sentences + e['id'][:-5] + '/', e['id'])
                e['spectrograms'] = get_spectrogram(path_to_wav_sentences + e['id'][:-5] + '/', e['id'])
...
import pickle
with open(data_path + '/../'+'data_collected_autoencoder.pickle', 'wb') as handle:
